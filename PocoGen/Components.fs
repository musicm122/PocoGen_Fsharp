...
open PocoGen.Validation

module Components =
    let getDbName (db: DbItem): string = db.Name
    let toDbItemPickerOption (dbs: DbItem list): string list = dbs |> List.map (getDbName)
    ...
    let formButtonDisabled text cmd =
        View.Button
            (verticalOptions = LayoutOptions.Fill,
             horizontalOptions = LayoutOptions.FillAndExpand,
             isEnabled = false,
    ...
    let formButtonEnabled text cmd =
        View.Button
            (verticalOptions = LayoutOptions.Fill,
             horizontalOptions = LayoutOptions.FillAndExpand,
             isEnabled = true,
    ...
    let connectionTestView (model: Model) dispatch =
        ...
        let testButton =
            (match model.CurrentFormState with
             ...
             | _ -> formButtonDisabled "Test" testConnection)
    ...
    let codeGenView (model: Model) dispatch =
        ...
        let langs: string list =
            model.Languages |> toLanguagePickerOption

        let loadDataEnabled = model |> HasAConnectionString
        ...
        let loadBtn: ViewElement =
            formButton "Load Db Connection Data" (fun () -> dispatch (FetchDatabases)) loadDataEnabled
